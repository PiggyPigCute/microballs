...
async def on_ready():
    ...
    for guild_id in spawn_channels:
        await bot.get_guild(int(guild_id)).get_channel(int(spawn_channels[guild_id])).send("Sorry, je suis tout cassé depuis quelques heures, les MicroBalls ci-dessus ne pourront plus être récupérer et normalement je re-fonctionne ^^")
    print("Let's go !")
